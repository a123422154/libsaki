...
void Sumire::handleDrawSelf(const Table &table, Mount &mount, bool rinshan)
{
    ...
    if (mWant.isYao()) {
        ...
        if (closed.ct(mWant) == 0) {
            ...
            for (int ti = 0; ti < 34; ti++) {
                T34 t(ti);
                mount.lightA(t, t == mWant ? 1000 : -40, rinshan);
            }
        } else if (hand.step() > 0) {
            recoverReady();
        }
        ...
        if (closed.ct(mWant.prev()) == 0) {
            ...
            for (int ti = 0; ti < 34; ti++) {
                T34 t(ti);
                mount.lightA(t, mWant | t ? 1000 : -40, rinshan);
            }
        } else {
            recoverReady();
        }
...
void Sumire::pickBullet(const Table &table, Mount &mount)
{
    ...
    auto niceMount = [&myHand, &mount](const T37 feed) {
        ...
        if (feed34.isYao()) {
            ...
            bool canGotL = mount.remainA(feed34.prev()) > 0;
            bool gotR = myHand.closed().ct(feed34.next()) > 0;
            bool canGotR = mount.remainA(feed34.next()) > 0;
            return (gotL || canGotL) && (gotR || canGotR);
        }
    };
    ...
    if (maxHappy > 0) {
        ...
        mount.loadB(mFeed, 1);
        // CONTINUE reserve isorider or clamper if not in hand
        table.popUp(mSelf);
    }
}
